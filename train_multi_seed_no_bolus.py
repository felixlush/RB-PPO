#!/usr/bin/env python3
"""
Launch multi-seed training runs for the no-bolus ablation variant.

Example:
    python train_multi_seed_no_bolus.py --steps 5_000_000
    python train_multi_seed_no_bolus.py --seeds 11 22 -- --debug
"""
...
DEFAULT_SEEDS: tuple[int, ...] = (101, 193, 102, 22, 408, 221, 900)
...
def launch_training(
    ...
    using a run name that identifies the no-bolus ablation.
    """
    for seed in seeds:
        timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
        run_name = f"RB-PPO-noBolus_seed{seed}"
        cmd = [
            sys.executable,
            str(script_path),
            "--type",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run RB-PPO no-bolus ablation with multiple seeds."
    )
    parser.add_argument(
        "--steps",
        type=int,
    ...
    parser.add_argument(
        ...
        help="Additional arguments forwarded to train_ppo_v2.py.",
    )
